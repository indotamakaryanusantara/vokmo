...
import { Code2, Filter, Mail, ShieldCheck, type LucideIcon } from "lucide-react";

type AboutPageCopy = Dictionary["pages"]["about"];
...
export function AboutPage({ copy, contactHref }: AboutPageProps) {
  return (
    <>
      ...
      <section aria-label={copy.pillarsAriaLabel} className="bg-background-muted py-12 sm:py-16 md:py-24">
        <Container>
          <ul className="grid min-w-0 gap-4 sm:gap-6 md:grid-cols-2">
            {copy.pillars.map((item) => {
              const Icon = pillarIcons[item.id] ?? Mail;
              return (
                <li key={item.id}>
      ...
      <section aria-label={copy.principlesAriaLabel} className="bg-background py-12 sm:py-16 md:py-24">
        <Container>
          <h2 className="font-heading text-2xl font-semibold tracking-tight text-slate-900 sm:text-3xl">
            {copy.principlesTitle}
          </h2>
          <ul className="mt-6 grid min-w-0 gap-4 sm:mt-8 sm:gap-6 md:grid-cols-3">
            {copy.principles.map((item) => (
              <li key={item.id}>
                <article className="flex h-full flex-col rounded-lg border border-border bg-white p-5 sm:p-6">
                  <h3 className="font-heading text-lg font-semibold tracking-tight text-slate-900">{item.name}</h3>
                  <p className="mt-2 text-pretty text-sm leading-relaxed text-muted sm:text-base">{item.summary}</p>
                </article>
              </li>
            ))}
